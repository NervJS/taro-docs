...
| start | `string` | 否 | 仅当 mode = time 或 date 时有效，表示有效时间范围的开始，字符串格式为"hh:mm" |
| end | `string` | 否 | 仅当 mode = time 或 date 时有效，表示有效时间范围的结束，字符串格式为"hh:mm" |
| onChange | `CommonEventFunction<ChangeEventDetail>` | 是 | value 改变时触发 change 事件 |
...
| start | `string` |  | 否 | 仅当 mode = time 或 date 时有效，表示有效时间范围的开始，字符串格式为"YYYY-MM-DD" |
| end | `string` |  | 否 | 仅当 mode = time 或 date 时有效，表示有效时间范围的结束，字符串格式为"YYYY-MM-DD" |
