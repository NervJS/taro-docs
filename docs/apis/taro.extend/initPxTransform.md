...
```tsx
(config: { baseFontSize?: number; deviceRatio?: TaroGeneral.TDeviceRatio; designWidth?: number; targetUnit?: string; unitPrecision?: number; }) => void
```
...
| 参数 | 类型 |
| --- | --- |
| config | `{ baseFontSize?: number; deviceRatio?: TaroGeneral.TDeviceRatio; designWidth?: number; targetUnit?: string; unitPrecision?: number; }` |
