...
Taro 移动端的开发基于 Facebook 的开源项目 [React Native](https://github.com/facebook/react-native)，使用 **React** 框架开发的 Taro 项目，经过适配可以打包成 Android 及 iOS APP。
...
React Native 我们将以最新版为主做兼容，并及时跟进社区发展。
...
使用 `taro init` 命令进行初始，选择默认模板。fork 壳工程代码 [Taro Native Shell 项目](https://github.com/NervJS/taro-native-shell)。
...
Taro 中对接口做了按需加载，不需要的原生依赖可以不进行安装，以降低 APP 包大小。如 APP 中不涉及音视频，可直接删除 expo-av 依赖。
