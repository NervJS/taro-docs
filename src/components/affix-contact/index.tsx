...
// props: 
//     pos: affix footer
export default function AffixContact (props) {
  const { pos } = props;
  ...
  return <div className={`affix-contact-container ${pos === 'footer' ? 'inline' : 'affix'}`}>
    {pos === 'affix' &&  <div
    ...
    {
      pos === 'footer' &&  <div className='affix-title-inline' onClick={toggleShowQrCode}>沟通反馈</div>
    }
    <div className={`affix-contact-content-wrap ${showQrCode ? 'active':''}`} onClick={toggleShowQrCode}>
      <div className={`affix-contact-content ${showQrCode ? 'active':''} ${pos === 'footer' ? 'inline' : 'affix'}`}>
